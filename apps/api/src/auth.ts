...
export class AuthService {
  ...
  private token(userId: string) {
    return sign({ sub: userId }, process.env.JWT_SECRET || 'development-only-secret', {
      expiresIn: '7d',
    });
  }
  ...
  async register(dto: Credentials) {
    ...
    try {
      ...
      const user = await client.query(
        ...
        [dto.email.toLowerCase(), hash],
      );
      const org = await client.query(
        'INSERT INTO organizations(name) VALUES($1) RETURNING id,name',
        [`${dto.email.split('@')[0]}'s workspace`],
      );
      ...
      return {
        token: this.token(user.rows[0].id),
        user: user.rows[0],
        organization: {
          ...org.rows[0],
          role: 'owner',
